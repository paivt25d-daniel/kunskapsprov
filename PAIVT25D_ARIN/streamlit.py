...
def preprocess_image(image):
    ...
    # Convert to numpy array WITHOUT dividing by 255
    ...
    if not st.session_state['models']:
        ...
        if uploaded_file is not None:
            if 'last_file_name' not in st.session_state or uploaded_file.name != st.session_state['last_file_name']:
                ...
                if 'prediction_plot' in st.session_state:
                    del st.session_state['prediction_plot']
        ...
        if uploaded_file is not None:
            ...
            with col2:
                ...
                st.image(processed_img.reshape(28, 28).astype("uint8"), width=200)

            # Scale the input
            scaler = st.session_state['scaler']
            scaled_img = scaler.transform(processed_img) if scaler else processed_img

            # Run predictions only once per upload
            if 'prediction_results' not in st.session_state:
                results = []

                for name, model in st.session_state['models'].items():
                    pred = model.predict(scaled_img)[0]

                    # Get probability if available
                    if hasattr(model, "predict_proba"):
                        prob = model.predict_proba(scaled_img)[0][pred]
                    else:
                        prob = 1.0
                ...
                st.session_state['prediction_results'] = pd.DataFrame(results)

            # Display prediction tables
            st.subheader("Predictions")
            st.dataframe(st.session_state['prediction_results'])

            # Determine most common predicted digit
            most_common_pred = st.session_state['prediction_results']["Prediction"].mode()[0]
            ...
            if 'prediction_plot' not in st.session_state:
                fig, ax = plt.subplots()
                ax.bar(
                    st.session_state['prediction_results']["Model"],
            ...
            st.pyplot(st.session_state['prediction_plot'])
